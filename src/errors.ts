import type { SecretStoreError, SecretStoreErrorCode } from '@ankhorage/contracts/secrets';
...
export function mapSupabaseVaultError(
  ...
  if (normalizedCode === 'P0002') {
    return createSecretStoreError('not_found', `Secret ${operation} target was not found.`);
  }

  if (normalizedCode === '23505') {
    return createSecretStoreError('conflict', 'A secret already exists for this scoped reference.');
  }
